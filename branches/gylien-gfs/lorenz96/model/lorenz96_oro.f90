...
!=======================================================================
!
! [PURPOSE:] Lorenz 1996 model with orography
!
! [PUBLIC:]
...
!=======================================================================
  ...
  INTEGER,SAVE,PUBLIC :: numwave=1          ! wavenumber of orography
! numwave=0 means no orography, for which lorenz96.f90 is more efficient
  REAL(r_size),SAVE,PUBLIC :: ampwave=2.0d0 ! amplitude of orography
CONTAINS
!=======================================================================
...
SUBROUTINE lorenz96_core(xin,xout)
  ...
  REAL(r_size),INTENT(IN) :: xin(1:nx)
  REAL(r_size),INTENT(OUT) :: xout(1:nx)
  REAL(r_size),ALLOCATABLE :: y(:)
  INTEGER :: i

  ALLOCATE( y(1:nx) )
  DO i=1,nx
    y(i) = xin(i) - ampwave * sin( 2.0 * pi * real(i*numwave) / real(nx) )
  END DO

  xout(1) = xin(nx) * ( y(2) - y(nx-1) ) - xin(1) + force
  xout(2) = xin(1) * ( y(3) - y(nx) ) - xin(2) + force
  DO i=3,nx-1
    xout(i) = xin(i-1) * ( y(i+1) - y(i-2) ) - xin(i) + force
  END DO
  xout(nx) = xin(nx-1) * ( y(1) - y(nx-2) ) - xin(nx) + force
...
SUBROUTINE TL_lorenz96_core(x9,xin,xout)
  ...
  REAL(r_size),INTENT(IN) :: xin(1:nx)
  REAL(r_size),INTENT(OUT) :: xout(1:nx)
  REAL(r_size),ALLOCATABLE :: y(:),y9(:)
  INTEGER :: i

  ALLOCATE( y(1:nx) )
  ALLOCATE( y9(1:nx) )
  DO i=1,nx
    y9(i) = x9(i) - ampwave * sin( 2.0 * pi * real(i*numwave) / real(nx) )
  END DO
  y = xin

!  xout(1) = xin(nx) * ( y(2) - y(nx-1) ) - xin(1) + force
  xout(1) = x9(nx) * ( y(2) - y(nx-1) ) + xin(nx) * ( y9(2) - y9(nx-1) ) - xin(1)
!  xout(2) = xin(1) * ( y(3) - y(nx) ) - xin(2) + force
  xout(2) = x9(1) * ( y(3) - y(nx) ) + xin(1) * ( y9(3) - y9(nx) ) - xin(2)
  DO i=3,nx-1
!    xout(i) = xin(i-1) * ( y(i+1) - y(i-2) ) - xin(i) + force
    xout(i) = x9(i-1) * ( y(i+1) - y(i-2) ) + xin(i-1) * ( y9(i+1) - y9(i-2) ) - xin(i)
  END DO
!  xout(nx) = xin(nx-1) * ( y(1) - y(nx-2) ) - xin(nx) + force
  xout(nx) = x9(nx-1) * ( y(1) - y(nx-2) ) + xin(nx-1) * ( y9(1) - y9(nx-2) ) - xin(nx)
...
!!  xout(nx) = xin(nx-1) * ( y(1) - y(nx-2) ) - xin(nx) + force
!  xout(nx) = x9(nx-1) * ( y(1) - y(nx-2) ) + xin(nx-1) * ( y9(1) - y9(nx-2) ) - xin(nx)
  y(1) = y(1) + x9(nx-1) * xout(nx)
  y(nx-2) = y(nx-2) - x9(nx-1) * xout(nx)
  xin(nx-1) = xin(nx-1) + ( y9(1) - y9(nx-2) ) * xout(nx)
  xin(nx) = xin(nx) - xout(nx)
!  DO i=3,nx-1
  DO i=nx-1,3,-1
!!    xout(i) = xin(i-1) * ( y(i+1) - y(i-2) ) - xin(i) + force
!  xout(i) = x9(i-1) * ( y(i+1) - y(i-2) ) + xin(i-1) * ( y9(i+1) - y9(i-2) ) - xin(i)
    y(i+1) = y(i+1) + x9(i-1) * xout(i)
    y(i-2) = y(i-2) - x9(i-1) * xout(i)
    xin(i-1) = xin(i-1) + ( y9(i+1) - y9(i-2) ) * xout(i)
    xin(i) = xin(i) - xout(i)
  END DO
!!  xout(2) = xin(1) * ( y(3) - y(nx) ) - xin(2) + force
!  xout(2) = x9(1) * ( y(3) - y(nx) ) + xin(1) * ( y9(3) - y9(nx) ) - xin(2)
  y(3) = y(3) + x9(1) * xout(2)
  y(nx) = y(nx) - x9(1) * xout(2)
  xin(1) = xin(1) + ( y9(3) - y9(nx) ) * xout(2)
  xin(2) = xin(2) - xout(2)

!!  xout(1) = xin(nx) * ( y(2) - y(nx-1) ) - xin(1) + force
!  xout(1) = x9(nx) * ( y(2) - y(nx-1) ) + xin(nx) * ( y9(2) - y9(nx-1) ) - xin(1)
  y(2) = y(2) + x9(nx) * xout(1)
  y(nx-1) = y(nx-1) - x9(nx) * xout(1)
  xin(nx) = xin(nx) + ( y9(2) - y9(nx-1) ) * xout(1)
  xin(1) = xin(1) - xout(1)
