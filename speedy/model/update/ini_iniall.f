...
C                                             ISTART=0 no restart
      READ (2,*) ISTART
!     IF (ISTART.NE.0) ISTART = 1                                       !TM
      IF (ISTART.EQ.1110) THEN ! restart and 6-hourly output            !TM
        ISTART = 1                                                      !TM
        IHOUT = 1                                                       !TM
        IPOUT = 1                                                       !TM
      ELSE IF (ISTART.EQ.1111) THEN ! Start with gridded data           !TM
        IHOUT = 1                                                       !TM
        IPOUT = 1                                                       !TM
      ELSE IF (ISTART.EQ.1112) THEN ! Start with gridded data           !TM
        ISTART = 1111                                                   !TM
        IHOUT = 1                                                       !TM
        IPOUT = 0                                                       !TM
      ELSE IF (ISTART.NE.0) THEN                                        !TM
        ISTART = 1                                                      !TM
        IHOUT = 0                                                       !TM
        IPOUT = 0                                                       !TM
      ELSE                                                              !TM
...
C--   6. Initialize time-mean arrays
      ...
      IF (IHOUT.NE.1) THEN !Do not call time-mean procedures if IHOUT=1 !TM
...
C--
     ...
     *             RADANG,PPL,'atdf',NORUN,IYEAR0,IMONT0)
      ...
      ELSE ! when IHOUT=1                                               !TM
        CALL IOGRID (5) ! create control file for 6-hourly output       !TM
