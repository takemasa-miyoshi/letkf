...
!  REAL(r_size),ALLOCATABLE :: tmp2k(:)
  ...
  WRITE(6,'(A)') 'Hello from set_common_obs_nhm'
...
!  DEALLOCATE( tmp2k )
  ...
  RETURN
END SUBROUTINE set_common_obs_nhm
!-----------------------------------------------------------------------
...
SUBROUTINE Trans_XtoY(elm,ri,rj,rk,v3d,v2d,yobs)
  ...
  CASE(id_ps_obs) ! PS
    CALL itpl_3d(v3d(:,:,:,iv3d_p),ri,rj,1.5d0,yobs)
  CASE(id_rh_obs) ! RH
    DO k=ks,ke
      DO j=js,je
        IF(ie <= nlon ) THEN
...
!-----------------------------------------------------------------------
SUBROUTINE phys2ijk(p,elem,rlon,rlat,rlev,ri,rj,rk)
  IMPLICIT NONE
  REAL(r_size),INTENT(IN) :: p(nlon,nlat,nlev)
  ...
  REAL(r_size) :: alon(nlon),alat(nlat)
...
!
  ...
  DO
    ...
    j0 = j
    DO i=1,nlon
      alon(i) = lon(i,j)*aj + lon(i,j-1)*(1.0d0-aj)
    END DO
    DO i=1,nlon
      IF(alon(i) < 0.0d0) CYCLE
      IF(alon(i) > rlon) EXIT
    END DO
    ai = (rlon - alon(i-1)) / (alon(i) - alon(i-1))
    ...
    IF(ri > HUGE(i)) THEN
      ...
      RETURN
    END IF
    IF(CEILING(ri) < 2 .OR. CEILING(ri) > nlon) THEN
      ri = 0.0d0
      rj = 0.0d0
      rk = 0.0d0
      RETURN
...
!
    i = CEILING(ri)
    DO j=1,nlat
      alat(j) = lat(i,j)*ai + lat(i-1,j)*(1.0d0-ai)
    END DO
    DO j=1,nlat
      IF(alat(j) < 0.0d0) CYCLE
      IF(alat(j) > rlat) EXIT
    END DO
    aj = (rlat - alat(j-1)) / (alat(j) - alat(j-1))
    ...
    IF(rj > HUGE(j)) THEN
      ...
      RETURN
    END IF
    IF(CEILING(rj) < 2 .OR. CEILING(rj) > nlat) THEN
      ri = 0.0d0
      rj = 0.0d0
      rk = 0.0d0
      RETURN
...
! rlev -> rk
!
  IF(NINT(elem) == id_ps_obs .OR.&
     NINT(elem) == id_rain_obs) THEN ! surface pressure observation
    rk = 1.5d0
  ELSE
    !
    ...
    DO k=1,nlev
      IF(i <= nlon) THEN
        lnps(i-1:i,j-1:j) = LOG(p(i-1:i,j-1:j,k))
      ELSE
        lnps(i-1,j-1:j) = LOG(p(i-1,j-1:j,k))
        lnps(1,j-1:j) = LOG(p(1,j-1:j,k))
      END IF
...
SUBROUTINE monit_mean(file)
  ...
  DO n=1,nobs
    ...
    IF(CEILING(rk) < 2 .AND. NINT(obselm(n)) /= id_ps_obs) THEN
      ...
      END IF
    END IF
    CALL Trans_XtoY(obselm(n),ri,rj,rk,v3d,v2d,hdxf)
    dep = obsdat(n) - hdxf
